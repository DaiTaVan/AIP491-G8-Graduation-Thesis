...
class GLEU :
    ...
    def __init__(self,n=4, tokenizer_func = lambda x: x.split()) :
        self.order = n
    ...
    def load_sources(self,spath) :
        self.all_s_ngrams = [ [ self.get_ngram_counts(self.tokenizer_func(line),n)
                                for n in range(1,self.order+1) ]
    ...
    def load_references(self,rpaths) :
        ...
        for rpath in rpaths :
            for i,line in enumerate(open(rpath)) :
                self.refs[i].append(self.tokenizer_func(line))
                self.rlens[i].append(len(self.tokenizer_func(line)))
