...
handler = RotatingFileHandler("logging/pipeline.log", maxBytes=5*1024*1024, backupCount=5)
...
class Pipeline:
    def __init__(
        ...
        # Initialize agents
        self.agent1 = Agent1(config=self.config, model=self.get_llm_from_config(self.config["Agent_1"]["llm"]))
        self.agent2 = Agent2(config=self.config, model=self.get_llm_from_config(self.config["Agent_2"]["llm"]))
        self.agent3 = Agent3(
            # llm=self.gpt_model, 
        ...
        self.agent5 = Agent5(model=self.get_llm_from_config(self.config["Agent_5"]["llm"]), config=self.config)
        self.agent6 = Agent6(model=self.get_llm_from_config(self.config["Agent_6"]["llm"]), config=self.config)

        # Create the LangGraph workflow
        self.graph = self._create_graph()
        self.logger.info("Pipeline initialization complete.")
    
    def get_llm_from_config(self, llm_config):
        llm = None
        if llm_config["type"] == "openai":
            llm = ChatOpenAI(model_name = llm_config["model"], temperature = llm_config["temperature"])
        elif llm_config["type"] == "ollama":
            llm = OllamaLLM(model = llm_config["model"], temperature = llm_config["temperature"])
        else:
            raise TypeError(f"Not support LLM type: {llm_config['type']}")
        return llm
